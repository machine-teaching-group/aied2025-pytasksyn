...
def summarize(data_path, annotations_1, annotations_2, metrics):
    ...
    for i, N in enumerate([1, 5, 10]):
            ...
            for technique, passed_tasks in passed_tasks_at_N.items():
                if technique == 'PyTaskSyn-50%':
                    ...
                    for concept in concepts:
                        ...
                        if len(passed_tasks) > 0:
                            concept_counts[concept].append(1)
                        else:
                            concept_counts[concept].append(0)
                    high_quality_counts = []
                    for task in passed_tasks:
                        ...
                        overall_by_expert = (qoverall_by_expert_1+qoverall_by_expert_2)/2
                        
                        # if qoverall_by_expert_1 == 1.0 and qoverall_by_expert_2 == 1.0 else 0.0
                        # if overall_by_expert == 1.0:
                        high_quality_counts.append(overall_by_expert)
                    
                    if len(passed_tasks) > 0:
                        avg_high_quality = sum(high_quality_counts) / len(high_quality_counts)
                        theme_counts_high_quality[theme].append(avg_high_quality)
                    else:
                        theme_counts_high_quality[theme].append(0)
                    for concept in concepts:
                        if len(passed_tasks) > 0:
                            avg_high_quality = sum(high_quality_counts) / len(high_quality_counts)
                            concept_counts_high_quality[concept].append(avg_high_quality)
                        else:
                            concept_counts_high_quality[concept].append(0)
        ...
        rects1 = ax1.bar(x - BAR_WIDTH/2, [v * 100 for v in avg_theme_counts_ordered.values()], BAR_WIDTH, label='Coverage (\%)', color='orange', alpha=0.95, edgecolor='black', linewidth=4, capsize=0, error_kw={'elinewidth': 6, 'capthick': 0}, hatch='.')
        rects2 = ax1.bar(x + BAR_WIDTH/2, [v * 100 for v in avg_theme_counts_high_quality_ordered.values()], BAR_WIDTH, label='Precision (\%)', color='dodgerblue', alpha=0.95, edgecolor='black', linewidth=4, capsize=0, error_kw={'elinewidth': 6, 'capthick': 0}, hatch='/')
        ...
        rects3 = ax2.bar(x - BAR_WIDTH/2, [v * 100 for v in avg_concept_counts_ordered.values()], BAR_WIDTH, label='Coverage (\%)', color='orange', alpha=0.95, edgecolor='black', linewidth=4, capsize=0, error_kw={'elinewidth': 6, 'capthick': 0}, hatch='.')
        rects4 = ax2.bar(x + BAR_WIDTH/2, [v * 100 for v in avg_concept_counts_high_quality_ordered.values()], BAR_WIDTH, label='Precision (\%)', color='dodgerblue', alpha=0.95, edgecolor='black', linewidth=4, capsize=0, error_kw={'elinewidth': 6, 'capthick': 0}, hatch='/')
        ...
        # Save and close plot
        plt.tight_layout()
        plt.savefig('plots/theme_and_concept_counts_precision_coverage.pdf', bbox_inches='tight')
        plt.close()
